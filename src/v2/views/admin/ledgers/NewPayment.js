...
function NewPayment() {
    ...
    const changeHandler = (e) => {
        if (e.target.name == 'company_id') {
            ...
            if (Number(e.target.value) <= (Location.state && Location.state.id != '' ? state.client_unused_credits : state.ledger_section_details.length > 0 && state.ledger_section_details[0].client_unused_credits)) {
    ...
    const paymentModesList = () => {
        CommonApi.paymentDropdown().then((res) => {
    ...
    const invoiceListing = (args) => {
        setLoading(true);
        PaymentsApi.invoiceListing('invoice', args).then((res) => {
            if (res.data.statusCode === 1003) {
                setLoading(false);
    ...
    const indexList = (args) => {
        setLoading(true);
        LedgerApi.indexApi('payment', args).then((res) => {
            setLoading(false);
            if (res.data.statusCode === 1003) {
                // state['ledger_section_details'] = res.data.data.ledger_section_details
                setState({
                    ...res.data.data,
                    ledger_section_details: res.data.data.ledger_section_details
                });
            } else {
    ...
    const updateSubmitHandler = () => {
        if (id !== '' && id !== null && id !== undefined) {
            ...
            LedgerApi.updatePayment('payment', id, state).then((res) => {
                if (res.data.statusCode === 1003) {
            ...
            LedgerApi.storePayment('payment', state).then((res) => {
                if (res.data.statusCode === 1003) {
                    setOpen(true);
                } else {
                    addErrorMsg(res.data.message);
    ...
    const columns = [
        {
            ...
            renderCell: (cellValues) => {
                return (
                    <>
                        {loading ? <Skeleton animation="wave" width="80px" height='25px' /> : <Text smallBlack>{cellValues.row.date}-{cellValues.row.due_date}</Text>}
                    </>
                )
            }
        },
        {
            ...
            renderCell: (cellValues) => {
                return (
                    <>
                        {loading ? <Skeleton animation="wave" width="80px" height='25px' /> : <Text smallBlack>{cellValues.row.invoice_id ? cellValues.row.invoice_id : '-'}</Text>}
                    </>
                )
            }
        },
        {
            ...
            renderCell: (cellValues) => {
                return (
                    <>
                        {loading ? <Skeleton animation="wave" width="80px" height='25px' /> : <Text smallBlack>{cellValues.row.hours ? `${cellValues.row.hours} Hrs` : '-'}</Text>}
                    </>
                )
            }
        },
        {
            ...
            renderCell: (cellValues) => {
                return (
                    <>
                        {loading ? <Skeleton animation="wave" width="80px" height='25px' /> : <Text smallBlack>{cellValues.row.amount}</Text>}
                    </>
                )
            }
        },
        ...
        },
        {
            ...
            renderCell: (params) => (
                <>
                    {loading ? <Skeleton animation="wave" width="80px" height='25px' /> :
                        <Input
                            formControlProps={{
                                fullWidth: true,
                            }}
        ...
        },
        {
            ...
            renderCell: (params) => {
                return (
                    <>
                        {loading ? <Skeleton animation="wave" width="80px" height='25px' /> : <Text smallBlack>{state.ledger_section_details[params.api.getRowIndex(params.row.id)].balance_amount}</Text>}
                    </>
                )
            }
        }
    ...
    return (
        <Grid container pl={15}>
            <Grid item lg={12}>
                <Breadcrumbs>
                    <Text largeLabel onClick={() => navigate('/ledger')} sx={{ cursor: 'pointer' }}>Ledger</Text>
                    <Text largeBlack>{actionState == 'view' || actionState == 'update' ? 'View' : 'New'} Payment</Text>
                </Breadcrumbs>
            </Grid>
            <Grid item container lg={12} justifyContent='center' pt={3}>
                <Grid item lg={10} justifyContent='end'>
                    <Box sx={{
                        ...
                        <Grid container p={4}>
                            <Grid item lg={12}>
                                <Text largeBoldBlack>{actionState == 'view' ? 'View' : 'New'} Payment</Text>
                            </Grid>
                            <Grid item container lg={12} pt={3} spacing={4}>
                                ...
                                <Grid item lg={4}>
                                    <Input
                                        ...
                                        clientInput
                                        labelText={<Text largeLabel>Client Unused Amount</Text>}
                                ...
                                <Grid item lg={4}>
                                    <Input
                                        ...
                                        handleChange={changeHandler}
                                        clientInput
                                        labelText={<Text largeLabel>Invoice Amount</Text>}
                                        disabled={actionState == 'view' ? true : false}
                                    />
                                ...
                                <Grid item lg={4}>
                                    <Input
                                        ...
                                        handleChange={changeHandler}
                                        clientInput
                                        labelText={<Text largeLabel>Received</Text>}
                                        disabled={actionState == 'view' ? true : false}
                                    />
                                    {error.total_ledger_amount ? <Text red>{error.total_ledger_amount ? error.total_ledger_amount : ''}</Text> : ''}
                                </Grid>
                                ...
                                <Grid item lg={4}>
                                    <Date
                                        labelText={<Text largeLabel>Received On</Text>}
                                        name='received_on'
                                ...
                                (state.company_id !== '' && state.ledger_section_details.length == 0) ?
                                    <Grid item lg={12} pt={5} textAlign='center'>
                                        ...
                                        <Grid item lg={12} pt={3}>
                                            {console.log(state.ledger_section_details, 'state.ledger_section_details')}
                                            <Table
                    ...
                    <Box textAlign='end' pt={3}>
                        <Button popupCancel onClick={() => navigate('/ledger')}>Cancel</Button>
                        {
                            actionState == 'view' ?
                                (LocalStorage.getUserData().super_admin || (rolePermission.some(item => item.slug == "invoice_edit" && item.is_allowed == true))) ?
            ...
            <ReusablePopup openPopup={open} setOpenPopup={setOpen} white statusWidth iconHide>
                <Box textAlign='center' p={2} pb={1}>
                    ...
                    <Button blueButton onClick={() => navigate('/ledger')}>Go To Home</Button>
                </Box>
            </ReusablePopup>
