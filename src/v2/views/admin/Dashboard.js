...
export default function Dashboard() {
    ...
    const storeReminderApi = () => {
        ...
        if (state.is_payroll_reminder === false) {
            state.reminders = []
        }
        DashboardAPI.storeReminder(state).then((res) => {
            if (res.data.statusCode == 1003) {
    ...
    const remaindersAction = (args) => {
        ...
        } else if (args.Header == 'Self Reminders') {
    ...
    const changeHandler = (e, index, args, value) => {
        if (args === 'recurring') {
            if (e.target.name === 'recurring_days') {
                state.reminders[index].recurring_days = e.target.value
            ...
            //     state.reminders[index].cycle = e.target.value
            //     setRemindDays(value.props.children[0]);
            //     setState({ ...state })
            // }
            else {
    ...
    const validateReminderErrors = () => {
        ...
        state.reminders.map((item) => {
            ...
            errors.recurring_days = state.is_payroll_reminder ? validate_emptyField(item.recurring_days) : '';
            err.push(errors);
            setErro(err);
        })
        return err;
    ...
    const changeTime = (value) => {
        ...
        const dateTime = moment.utc(event.target.value).format('HH:mm');
        console.log(dateTime, "fghgh");
        setState({
    ...
    const handleCancel = () => {
        setOpenReminder(false)
        setState({...state, is_payroll_reminder:false})
        setError({})
        setErro([])
        setState({
            name: '',
    ...
    return (
        <Grid container spacing={2} pl={15} pt={2} alignItems='flex-start'>
            ...
            <ReusablePopup openPopup={openReminder} setOpenPopup={setOpenReminder} white widthMax iconHide>
                <Grid container spacing={2}>
                        ...
                        state.documents.map((item, index) => (
                            <Grid item lg={12} md={12} sm={12} xs={12}>
                                <FileInput
                                    ...
                                    actionState={item.docName ? 1 : 'Upload'}
                                    ...
                                    <Grid item container spacing={2} lg={12} md={11} sm={11} xs={11}>
                                        <Grid item lg={3} md={3} sm={3} xs={12}>
                                            <SearchSelect
                                                ...
                                                disabled={state.is_payroll_reminder == true}
                                            />
                                        ...
                                        <Grid item lg={3} md={3} sm={3} xs={3}>
                                            <SearchSelect
                                                options={days}
                                                labelText={<Text largeLabel>Cycle</Text>}
                                                name='cycle'
                                                value={item.cycle}
                                                onChange={(e, value) => changeHandler(e, index, 'recurring', value)}
                                                remindDays={true}                                               
                                            />
                                        ...
                                        </Grid>
                                        {
                                            ...
                                            <Grid item lg={3} md={3} sm={3} xs={12}>
                                                ...
                                                {/* <SearchSelect
                                                    ...
                                                    onChange={(e) => changeHandler(e, index, 'recurring')}
                                                /> */}
                                                {erro.length > 0 && (<Text red>{erro[index] ? erro[index].recurring_days : ""}</Text>)}
                                            </Grid>
                                        }
                                    </Grid>
                    ...
                    <Grid item container spacing={2} lg={12} md={12} sm={12} xs={12} mt={1}>
                        <Grid item lg={6} md={6} sm={6} xs={12}>
                            <Button cancelText onClick={() => handleCancel()}>Cancel</Button>
                        </Grid>
                        <Grid item lg={6} md={6} sm={6} xs={12}>
