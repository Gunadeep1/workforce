...
const Drafted = () => {
    ...
    const [draftedArray, setDraftedArray] = useState([]);
    ...
    const handleGeneratePayroll = (status, name, id, from_date, to_date) => {
        ...
        PayrollApi.payRollGenerate(data).then((response) => {
            if (response.data.statusCode == 1003) {
                ...
                navigate('/payroll-view', { state: { status: status, name: name, payroll_configuration_id: id, from_date: from_date, to_date: to_date, viewAll : 'drafted' } })
            } else {
                setGenerateLoad(0);
        ...
        <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', flexDirection: 'column' }}>
            ...
            <Box sx={{ width: '80%' }} >
                <Box style={{ padding: '20px 10px 10px 10px' }}>
                    <Breadcrumbs aria-label="breadcrumb">
                        ...
                        <Text className={classes.navText2}>Drafted</Text>
                    </Breadcrumbs>
                </Box>
            </Box>
            <Box sx={{ width: '60%' }} pt={2} >
                ...
                <Box style={{ width: '100%' }}>
                    <Box style={{ width: '100%' }}>
                        {
                            draftedArray ? draftedArray.length > 0 ? draftedArray.map((obj, index) => (
                                <>
                                            ...
                                            <Stack direction='row' spacing={2} width='100%' >
                                                <Box className={classes.AccordionSummaryBox2} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                                    ...
                                                    <Box className={classes.flexBox} gap={4} >
                                                        <Box className={classes.flexBox} gap={1} style={{ cursor: !generateLoad ? 'pointer' : 'not-allowed' }} onClick={!generateLoad ? () => handleGeneratePayroll('Drafted', obj.pay_config_setting_name, obj.id, obj.from_date, obj.to_date) : undefined}>
                                                        ...
                                                        {/* <Box className={classes.flexBox}
                                                            onClick={() => handleGeneratePayroll('Drafted', obj.pay_config_setting_name, obj.id, obj.from_date, obj.to_date)}                                                            // onClick={() => navigate("/payroll-view", { state: { name: obj.pay_config_setting_name, status: 'Drafted', payroll_configuration_id: obj.id, from_date: obj.from_date, to_date: obj.to_date } })}
                                                        ...
                                                        <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', padding: '10px' }}>
                                                            <img onClick={() => playSkip(obj.id)} src={Play} alt="play" />
                                                        </Box> */}
                                                    </Box>
                                                </Box>
                                            </Stack>
