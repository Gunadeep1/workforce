...
export default function PayrollView(props) {
    ...
    function calculateTotalNetAmount(array) {
        return array.reduce((total, item) => Number(total) + (Number(item.net_amount) || 0), 0);
    }

    const DownloadReportAPI = () => {
        PayrollApi.downloadReport(employeeReport.payroll_configuration_id, employeeReport.employee_id).then((response) => {
            if (response.data.statusCode == 1003) {
                FileSaver.saveAs(response.data.payroll.data.pdf_link);
            } else {
                addErrorMsg(response.data.message);
            }
            ...
            <Box width={'750px'} height={'auto'}>
                <Box height={'10vh'} display={'flex'} alignItems={'center'} padding={'40px 40px 20px 40px'} justifyContent={'space-between'} >
                    <Typography sx={{
                        ...
                        fontWeight: "500",
                        color: "#262626"
                    }}>Employee Report - {employeeReport.employee_name}</Typography>
                    <Box display={'flex'} gap={1} sx={{ cursor: 'pointer' }} onClick={DownloadReportAPI}>
            ...
            main_row_data: [
                ...
                },
                {
                    ...
                    value: data.placement_information.map(info => (
                        info.payroll_information.length > 0 ?
                            info.payroll_information.map((payValue) =>
                                <Text sx={{ fontSize: "12px !important", fontFamily: "Nunito , Nunito Sans, sans-serif !important", fontWeight: '500 !important', color: "#171717 !important", paddingBottom: '10px !important' }}>{payValue.hours}</Text>
                            )
                            :
                ...
                },
                {
                    ...
                    value: data.placement_information.map(info => (
                        info.payroll_information.length > 0 ?
                            info.payroll_information.map((payValue) =>
                                <Text sx={{ fontSize: "12px !important", fontFamily: "Nunito , Nunito Sans, sans-serif !important", fontWeight: '500 !important', color: "#171717 !important", paddingBottom: '10px !important' }}>$ {payValue.total_amount_payable ? payValue.total_amount_payable : 0}</Text>
                            )
                            :
                ...
                },
                {
                    column: 'Salary Amount', value:
                        <>
                            {
                                data.timesheet_approval_pending ?
                                    <BlackToolTip arrow placement='top' title={<Text smallWhite>Timesheets are not Approved/Submitted</Text>}>
                ...
                { column: 'Net Payable Amount', value: <Text sx={{ fontSize: "12px !important", fontFamily: "Nunito , Nunito Sans, sans-serif !important", fontWeight: '500 !important', color: "#171717 !important" }}>$ {data.net_amount && Number(data.net_amount).toFixed(2)}</Text> },
                { column: 'Balance', value: <Text sx={{ fontSize: "12px !important", fontFamily: "Nunito , Nunito Sans, sans-serif !important", fontWeight: '500 !important', color: "#171717 !important" }}>$ {data.available_balance_amount.toFixed(2)}</Text> },
                ...
                { column: 'Expense Deductionn', value: <Text sx={{ fontSize: "12px !important", fontFamily: "Nunito , Nunito Sans, sans-serif !important", fontWeight: '500 !important', color: "#171717 !important" }}>$ {data.debit_expense_available}</Text> },
            ],
        ...
        const mainRowData = {
            ...
            main_row_data: [
                ...
                },
                {
                    column: 'Hours',
                    value: data.placement_information.map(info => (
                        <Text sx={{ fontSize: "12px !important", fontFamily: "Nunito , Nunito Sans, sans-serif !important", fontWeight: '500 !important', color: "#171717 !important" }}>{info.payroll_information.length > 0 ? info.payroll_information[0].hours : 0}</Text>
                    ))
                },
                {
                    column: 'Salary Amount', value:
                ...
                { column: 'Net Payable Amount', value: <Text sx={{ textAlign: 'center', fontSize: "12px !important", fontFamily: "Nunito , Nunito Sans, sans-serif !important", fontWeight: '500 !important', color: "#171717 !important" }}>$ {data.net_amount && Number(data.net_amount).toFixed(2)}</Text> },
            ],
            placement_information: data.placement_information.length
