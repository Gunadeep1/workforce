...
export default function InvoicesDashboard() {
    ...
    useEffect(() => {
        ...
        // console.log(tableLoading, "  Test in UseEffect");
    ...
    const getDashBoardInfoApi = () => {
        setLoading(true);
        InvoicesApi.getDashBoardInfoApi().then((res) => {
            setTimeout(() => {
        ...
        InvoicesApi.getAllInvoicesList(filterData, paginationData).then((res) => {
            setTimeout(() => {
                if (res.data.statusCode === 1003) {
                    ...
                    setInvoiceRowData(res.data.data);
    ...
    //             break
    //         case 'from_date':
    //             error.from_date = validate_emptyField(input.value)
    //             break
    ...
    const handleExportSubmit = () => {
        ...
        InvoicesApi.exportInvoice(exportData, LocalStorage.getAccessToken()).then(res => {
            if (res.data.statusCode == 1003) {
    ...
    const searchAPICall = (text) => {
        InvoicesApi.getAllInvoicesList({ ...filter, search: text }, { ...pagination, currentPage: 1, limit: 4 }).then((res) => {
            setTableLoading(false)
            if (res.data.statusCode === 1003) {
                setInvoiceRowData(res.data.data);
            }
    ...
    const handleSelect = (e) => {
        ...
        invoiceListingApi({ from_date: fromDate, to_date: toDate, search: '', status: '' }, { perPage: 4, currentPage: 1 });
        setPagination({
            total: "",
            currentPage: 1,
            perPage: 4,
    ...
    const chartData = [
        { value: dashboardInfo.raised_amount, color: '#3992F5', label: 'Amount Raised' },
        { value: dashboardInfo.received_amount, color: '#FFB126', label: 'Amount Recieved' },
        { value: dashboardInfo.overdue_amount, color: '#77D2B7', label: 'Overdue' },
    ]
    ...
    const closeBtn = () => {
        ...
        setTableLoading(true);
        setTimeout(() => {
            InvoicesApi.getAllInvoicesList({ ...filter, search: "" }, { ...pagination, currentPage: 1, limit: 4 }).then((res) => {
                setTableLoading(false)
                if (res.data.statusCode === 1003) {
                    setInvoiceRowData(res.data.data);
                }
    ...
    const handleChangePagination = (e, page) => {
        ...
        invoiceListingApi(filter, paginationData);
    }
    ...
    const columns = [
        ...
        },
        {
            ...
            align: "left",
            headerAlign: "left",
            headerName: "Invoice Id", sortable: false,
            disableColumnMenu: true,
            flex: 1,
            renderCell: (params) =>
                tableLoading ?
                    ...
                    (LocalStorage.getUserData().super_admin || (rolePermission.some(item => item.slug == "invoice_view" && item.is_allowed == true))) ?
                        <Text sx={{ font: '12px Nunito', fontWeight: `${500}`, color: `${params.row.label === 'Drafted' ? '#D97706' : '#16A34A'}`, cursor: 'pointer', }} onClick={() => navigate('/sales/invoices/invoices-history', { state: { data: params.row, pagination: pagination, } })}>{params.row.invoice_id ? params.row.invoice_id : "--"}</Text> :
        ...
        <Box className={classes.containerMain} >
            <Box width={'100%'} mb={'20px'} sx={{ display: 'flex', justifyContent: 'space-between' }}>
                <Breadcrumbs aria-label="breadcrumb">
                    ...
                    <Link to='' className={classes.navText2}><Text className={classes.navText1}>Invoices</Text></Link>
                </Breadcrumbs>
                <Box sx={{ display: 'flex', gap: '20px' }}>
                    {
                        (LocalStorage.getUserData().super_admin || (rolePermission.some(item => item.slug == "invoice_payment_view" && item.is_allowed == true))) ?
                            <Button addNew startIcon={<Plus />} onClick={() => navigate('/sales/add-invoices')} sx={{ height: '36px !important', minWidth: '144px !important', fontWeight: `${400} !important`, padding: '0px 20px !important' }}>Add Invoice</Button> :
                            <Button addNewDisable sx={{ height: '36px !important', minWidth: '144px !important', fontWeight: `${400} !important`, padding: '0px 20px !important' }}><img src={disablePlus} alt='add' style={{ marginRight: '10px' }} />Add Invoice</Button>
                    }
                    <button
                        type="button"
                                ...
                                <Stack direction={'row'} mb={'6px'} gap={2} >
                                    ...
                                    <Stack direction={'column'} mb={'6px'} >
                                        {loading ?
                                            ...
                                            :
                                            <>
                                                <Text sx={{ font: '13px Nunito', fontWeight: `${500}`, color: '#404040' }}>Partially paid invoices</Text>
                                ...
                                <Stack direction={'row'} mb={'6px'} gap={2} >
                                    ...
                                    <Stack direction={'column'} mb={'6px'} >
                                        ...
                                        {loading ?
                                            ...
                                            :
                                            <>
                                                <Text sx={{ font: '13px Nunito', fontWeight: `${500}`, color: '#404040' }}>Invoice awaiting approval</Text>
                                                <Text sx={{ font: '22px Nunito', fontWeight: `${600}`, color: '#171717' }}>{dashboardInfo.awaiting_approval ? dashboardInfo.awaiting_approval : '--'}</Text>
                                            </>
                                        }
                                    </Stack>
                                </Stack>
                            ...
                            <Grid container spacing={0} >
                                <Grid container item lg={3} md={3} sm={6} xs={6} alignItems={'space-between'} justifyContent={'space-between'} direction={'column'}>
                                    ...
                                    {loading ?
                                        ...
                                        :
                                        <Text sx={{ font: '22px Nunito', fontWeight: `${600}`, color: '#171717', }}>Invoice Amount</Text>}
                                        ...
                                        <Stack direction={'row'} mb={'6px'}>
                                            {loading ?
                                                ...
                                                <>
                                                    ...
                                                    <Text sx={{ font: '13px Nunito', fontWeight: `${500}`, color: '#404040' }}>Amount Raised</Text>
                                                </>
                                            }
                                        </Stack>
                                        ...
                                        <Stack direction={'row'} mb={'6px'}>
                                            {loading ?
                                                ...
                                                <>
                                                    ...
                                                    <Text sx={{ font: '13px Nunito', fontWeight: `${500}`, color: '#404040' }}>Amount Received</Text>
                                                </>
                                            }
                                        </Stack>

                                        {loading ? <Skeleton variant="text" sx={{ fontSize: '18px', width: '90px' }} /> : <Text sx={{ font: '18px Nunito', fontWeight: `${600}`, color: '#171717' }}>{`${getCurrencySymbol()} ${dashboardInfo.received_amount ? dashboardInfo.received_amount : '--'}`}</Text>}
                                    </Box>
                                </Grid>
                                ...
                                <Grid item container lg={3} md={3} sm={6} xs={6} justifyContent={{ lg: 'end' }} pr={{ lg: 1 }} mt={{ lg: 0, md: 0, sm: 5, xs: 5 }}>
                                    <Box sx={{ height: '115px', width: '115px' }}>
                                        {loading ? <Skeleton variant="circular" height={115} width={115} /> : <CustomPieChart data={chartData} />}
                                    </Box>
                ...
                <Grid item >
                    <div className={classes.searchField}>
                        <input
                            ...
                            placeholder="Search By Client Name / Invoice ID"
            ...
            <Box sx={{ width: '100%' }} mt={5}>
                <Table
                    height={'336px'}
                    rows={tableLoading ? [{ "id": 1 }, { "id": 2 }, { "id": 3 }, { "id": 4 }] : invoiceRowData}
                    columns={columns}
                    hideFooter={true}
                    hidePagination={true}
                ...
                <Box sx={{ width: "100%", display: 'flex', justifyContent: 'end', marginTop: "31px" }}>
                    <Stack spacing={2} >
                        ...
                        <StyledPagination
                            ...
                            disabled={invoiceRowData.length > 0 ? false : true}
