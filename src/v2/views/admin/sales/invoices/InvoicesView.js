...
import { Box, Breadcrumbs, Grid, Stack, SwipeableDrawer, Menu, MenuItem, Divider, Tab, Tabs, Avatar, Chip, TextField, Checkbox, } from "@mui/material";
import Button from '../../../../components/customButton/Button';
import KeyboardArrowDownIcon from '@mui/icons-material/KeyboardArrowDown';
import KeyboardArrowUpIcon from '@mui/icons-material/KeyboardArrowUp';
import InvoicesDashboardStyles from './InvoicesDashboardStyles';
...
import { addSuccessMsg, addWarningMsg, getCurrencySymbol } from '../../../../utils/utils';
...
import { validate_emptyField, validates_Integer, validates_emailId, isValid } from "../../../../components/Validation";
...
export default function InvoicesView() {
    ...
    const [selectValue, setSelectValue] = useState('Record Payment');
    ...
    const invoice_id = location.state ? location.state.data.id ? location.state.data.id : '' : '';
    ...
    const [writeOffDropdown, setWriteOffDropdown] = useState([]);
    const [statusDataError, setStatusDataError] = useState({});
    ...
    useEffect(() => {
        ...
        // console.log(location.state.pagination,  "   location.state.pagination location.state.pagination");

        // getClientInvoicesList({ ...filter, company_id: company_id }, location.state.pagination ? location.state.pagination : pagination);
        invoiceListingApi(filter, location.state.pagination ? { ...location.state.pagination, currentPage: 1, perPage: math.multiply(location.state.pagination.currentPage, location.state.pagination.perPage) } : pagination);
        getInvoiceIndexApi(invoice_id);
        getWriteOffDropdown();
        getPaymentModeDropdown();
        // eslint-disable-next-line  
    }, [])

    const getInvoiceIndexApi = (id) => {
        setLoading(true);
        setSelected(id);
        InvoicesApi.getInvoiceIndexApi(id).then((res) => {
            setTimeout(() => {
    ...
    const invoiceListingApi = (filterData, paginationData) => {
        setListLoading(true);
        InvoicesApi.getAllInvoicesList(filterData, paginationData).then((res) => {
            setTimeout(() => {
                if (res.data.statusCode === 1003) {
    ...
    const getClientInvoicesList = (filterData, paginationData) => {
        setListLoading(true);
        setFilter(filterData);
        filterData.company_id = invoiceForm.company_id
        InvoicesApi.getClientInvoicesList(filterData, paginationData).then((res) => {
            setTimeout(() => {
                if (res.data.statusCode === 1003) {
                    setListLoading(false);
    ...
    const updateInvoiceStatus = (data) => {
        InvoicesApi.updateInvoiceStatus(data).then((res) => {
            if (res.data.statusCode === 1003) {
                addSuccessMsg(res.data.message)
                getInvoiceIndexApi(selected === null ? invoice_id : selected);
                getClientInvoicesList({ ...filter, company_id: company_id }, pagination);
                // getInvoiceIndexApi(selected === null ? invoice_id : selected);
            } else {
    ...
    const getWriteOffDropdown = () => {
        InvoicesApi.getWriteOffDropdown('').then((res) => {
            if (res.data.statusCode === 1003) {
                setWriteOffDropdown(res.data.data)
            } else {
                addErrorMsg(res.data.message)
            }
        })
    ...
    const storeInvoicePayment = (data) => {
        const obj = {
        ...
        InvoicesApi.storeInvoicePayment(data).then((res) => {
            if (res.data.statusCode === 1003) {
                addSuccessMsg(res.data.message)
                getInvoiceIndexApi(selected === null ? invoice_id : selected);
                getClientInvoicesList({ ...filter, company_id: company_id }, pagination);
    ...
    const downloadDoc = (fileUrl) => {
        InvoicesApi.downloadInvoice(selected).then((res) => {
            if (res.data.statusCode === 1003) {
    ...
    const loadeMore = () => {
        setPagination({ perPage: parseInt(pagination.perPage) + 4 })
        if (cardTab === 0) {
            getClientInvoicesList(filter, { perPage: parseInt(pagination.perPage) + 4, currentPage: 1 });
        }
        if (cardTab === 1) {
            invoiceListingApi(filter, { perPage: parseInt(pagination.perPage) + 4, currentPage: 1 });
        }

    }

    const referenceIDValid = (event) => {
        const keyCode = event.keyCode || event.which
        const keyValue = String.fromCharCode(keyCode)
        const isValid = new RegExp('[a-zA-Z0-9-]').test(keyValue)
        if (!isValid) {
    ...
    const handleStatus = (args) => {
        ...
        if (args === 'mark_as_recurring') {
            // if (["Approved", "Paid", "Partially Paid"].includes(invoiceForm.status)) {
            //     setPopup('recurring');
            //     setOpen(true);
            // } else {
            //     addWarningMsg(`Performing invoice recurring is not permitted when the invoice status is ${invoiceForm.status}.`);
            // }
    ...
    const handleRecurring = (e) => {
        if (e.target.name == 'recurring_never_expires') {
            ...
            recurringData['custom_recurring_cycle_number'] = e.target.value == 1 || e.target.value == 4 || e.target.value == 8 ? 1 : e.target.value == 2 || e.target.value == 5 || e.target.value == 9 ? 2 : e.target.value == 3 || e.target.value == 6 || e.target.value == 10 ? 3 : e.target.value == 7 ? 6 : e.target.value == 11 ? '' : ''
    ...
    const handleRecordChange = (e) => {
        ...
        if (input.name === 'total_received_amount') {
            ...
            if (regex.test(e.target.value) || e.target.value == "") {
                ...
                if (input.value == "") {
                    setpaymentRecordData({ ...paymentRecordData, [input.name]: input.value, total_balance_amount: parseInt(0).toFixed(2), total_excess_amount: parseInt(0).toFixed(2) })
                } else {
                    if (value > paymentRecordData.amount_due) {
                        let excessAmount = math.subtract(value, paymentRecordData.amount_due);
                        setpaymentRecordData({ ...paymentRecordData, [input.name]: value, total_balance_amount: parseInt(0).toFixed(2), total_excess_amount: excessAmount.toFixed(2) })
                    } else {
                        let totalBalanceAmount = math.subtract(paymentRecordData.amount_due, value);
                        setpaymentRecordData({ ...paymentRecordData, [input.name]: value, total_balance_amount: totalBalanceAmount.toFixed(2), total_excess_amount: parseInt(0).toFixed(2) })
                    }
                }
            }
        }
    ...
    const uploadDocs = (e, index) => {
        if (e.target.files[0].type.split('/').some(r => ['png', 'jpg', 'jpeg'].includes(r))) {
            ...
            CommonApi.documentUpload("invoice-document",fileData, LocalStorage.getAccessToken())
                .then((res) => {
                    if (res.data.statusCode === 1003) {
    ...
    const handleWriteOffValidation = (e) => {
        let input = e.target;
        let err = error;
        switch (input.name || input.tagName) {
            case "write_off_id":
                err.write_off_id = validate_emptyField(input.value);
                break;
            default:
                break;
        }
        setStatusDataError(err);
    }
    ...
    const validateAll = () => {
        ...
        let errors = {};
        // errors.recurring_cycle_number = validates_Integer(recurring_cycle_number);
        ...
        // errors.record_payment = validate_emptyField(record_payment);
        errors.recurring_start_date = validate_emptyField(recurring_start_date);
        errors.recurring_end_date = recurringData.recurring_never_expires == true ? '' : validate_emptyField(recurring_end_date);
        return errors;
    ...
    const validateWriteOff = () => {
        let { write_off_id } = statusData;
        let errors = {};
        errors.write_off_id = validate_emptyField(write_off_id);
        return errors;
    ...
    const handleRecordPaymentSave = () => {
        ...
        if (isValid(errors)) {
            setOpen(false);
            storeInvoicePayment(paymentRecordData);
        } else {
            setError(errors);
        }
    }
    const handleWriteOffSubmit = () => {
        let errors = validateWriteOff();
        if (isValid(errors)) {
            handlePrompt('write_off')
            setOpen(false);
            invoiceListingApi(filter, pagination)
        } else {
            setStatusDataError(errors);
        }
    }
    ...
    const handleChangeMails = (e, target) => {
        ...
        if (newArr.filter((i) => i === value).length == 0) {
            newArr.push(value);
    ...
    const handleTabApis = (newValue) => {
        if (newValue === 0) {
            ...
            getClientInvoicesList({ ...filter, status: '' }, { perPage: 4, currentPage: 1 });
        }
        else if (newValue === 1) {
            setIndication(false);
            setFilter({ ...filter, status: '' })
            invoiceListingApi({ ...filter, status: '' }, { perPage: 4, currentPage: 1 });
        }
    }
    ...
    const handlePrompt = (status) => {
        ...
        if (status === 'clone') {
            navigate('/sales/add-invoices', { state: { id: selected, action: status, pagination: pagination, company_id: location.state.data.company_id } })
        }
        else if (status === 'write_off') {
            updateInvoiceStatus({ ...data, write_off_id: statusData.write_off_id });
        }
        else if (status === 'mark_as_recurring') {
            updateInvoiceStatusAsRecurring(status);
        }
        else if (status === 'reject') {
            updateInvoiceStatus({ ...data, reject_reason: rejectForm.reject_reason });
        }
        else {
    ...
    const handleListStatus = (args) => {
        ...
        if (cardTab == 0) {
            getClientInvoicesList({ ...filter, status: args }, { perPage: 4, currentPage: 1 });
        } else {
            invoiceListingApi({ ...filter, status: args }, { perPage: 4, currentPage: 1 });
        }
    ...
    const CommonPromptView = () => {
        ...
        return (<Stack p={'32px'} alignItems={'center'} width={'457px'} gap={'32px'}>
            ...
            {
                prompt == 'mark_as_sent' ? <Text className={classes.heading2}>Mark This Invoice As <span className={classes.heading16}>Sent</span></Text> : null
            }
            {
                prompt == 'clone' ? <Text className={classes.heading2}>Are You Sure To <span className={classes.heading16}>Clone</span> This Invoice? </Text> : null
            }
            {
                prompt == 'convert_to_draft' ? <Text className={classes.heading2}>Are You Sure Convert This Invoice To <span className={classes.heading16}>Draft ?</span></Text> : null
            }
            {
                prompt == 'stop_recurring' ? <Text className={classes.heading2}>Stop Recurring For This Invoice</Text> : null
            }
            {
                prompt == 'void' ? <Text className={classes.heading2}>Mark This Invoice As <span className={classes.heading16}>Void</span></Text> : null
            }
        ...
        return (
            <Box sx={{ width: '828px !important', padding: '30px' }}>
                <Stack direction={'row'} width={'100%'} justifyContent={'space-between'} mb={4} >
                    <Text className={classes.heading1}>{`Recurring of ${invoiceForm.reference_id ? invoiceForm.reference_id : ''}`}</Text>
                    ...
                    <Grid item lg={6} md={6} sm={12} xs={12}>
                        <Box sx={{ height: '80px', }}>
                            <DatePicker
                                ...
                                labelText={'To'}
                                disabled={recurringData.recurring_never_expires == true}
                                minDate={recurringData.recurring_start_date !== '' ? recurringData.recurring_start_date : ''}
                            />
                            <Text errorText> {recurringData.recurring_never_expires == false && recurringError.recurring_end_date ? recurringError.recurring_end_date : ""}</Text>
                        </Box>
                    </Grid>
                    {
                        recurringData.recurring_cycle_type == '11' &&
    ...
    const WriteOffView = () => {
        return (
            <Box sx={{ width: '442px', p: '16px 30px 27px 30px' }}>
                <Stack direction={'row'} width={'100%'} justifyContent={'end'}>
                    <CloseIcon onClick={() => handleClose()} style={{ cursor: 'pointer' }} />
                </Stack>
                <Stack direction={'row'} width={'100%'} justifyContent={'start'} mb={'20px'}>
                    <Text className={classes.heading1}>Write Off Reason?</Text>
                </Stack>
                ...
                <Box sx={{ height: '80px' }}>
                    <SearchSelect
                        ...
                        name={'write_off_id'}
                        value={statusData.write_off_id}
                        options={writeOffDropdown}
                    />
                    <Text errorText> {statusDataError.write_off_id ? statusDataError.write_off_id : ""}</Text>
                </Box>
                <Stack direction={'row'} width={'100%'} justifyContent={'end'}>
                    <Button blueBtnSave onClick={() => handleWriteOffSubmit()}>Submit</Button>
                </Stack>
            </Box>
        )
    }

    const InvoiceMailView = () => {
        return (
            <Box sx={{ width: '733px !important', padding: '25px 0px' }}>
                <Stack direction={'row'} width={'100%'} justifyContent={'space-between'} mb={3} px={'25px'} >
                    <Stack direction={'row'} gap={1} alignItems={'center'}> <Text className={classes.heading2}>Send Email to</Text><Text className={classes.heading14}>( Louis Vuitton)</Text></Stack>
                    <CloseIcon onClick={() => handleClose()} style={{ cursor: 'pointer' }} />
                </Stack>

                <Box px={'25px'}>
                    <Box sx={{ maxWidth: '578px' }} mb={3}>
                        <Input
                            formControlProps={{
                                fullWidth: true
                            }}
                            ...
                            labelText={'From'}
                            handleChange={handleInvoiceForm}
                            clientInput

                        />
                    </Box>
                    <Stack width={'100%'} direction={'row'} justifyContent={'space-between'} pr={3} >
                        <Box sx={{ width: '578px', height: '80px' }}>
                            <Autocomplete
                                limitTags={2}
                                multiple
                                freeSolo
                                ...
                                onChange={(e) => handleChangeMails(e, 'to')}
                                renderTags={(value) =>
                                    value.map((option, key) => (
                                        <Chip
                                            key={key}
                                            label={option}
                                            onDelete={() => handleDeleteChip(key, 'to')}
                            ...
                            />
                            <Text errorText> {error.to ? error.to : ""}</Text>

                        </Box>
                        <Text className={classes.heading2} sx={{ mt: 2 }}>CC / BCC</Text>
                    </Stack>
                </Box>
                <Divider />
                <Stack direction={'row'} justifyContent={'center'} width={'100%'} >
                    <input className={classes.subjectInput} placeholder='Subject' onChange={handleInvoiceForm} value={formData.subject} name='subject' />
                </Stack>
                <Divider />
                <Stack direction={'row'} justifyContent={'center'} width={'100%'} mt={3} height={'140px'}>
                    <textarea cols="40" rows="5" className={classes.descriptionInput} placeholder='Description' onChange={handleInvoiceForm} value={formData.description} name='description' ></textarea>
                </Stack>
                <Box sx={{ width: '100%', height: '66px', px: '30px', display: 'flex', justifyContent: 'start' }} mb={'25px'}>
                    <Stack className={classes.pdfBox}>
                        <img src={PdfIcon} height={42} width={42} alt='icons' />
                        <Stack gap={1}>
                            <Text className={classes.heading15}> {'INV-167.pdf'}</Text>
                            <Text className={classes.heading6}> {'800.KB- Uploded'}</Text>
                        </Stack>
                    </Stack>
                </Box>
                <Divider />
                <Stack direction={'row'} justifyContent={'space-between'} width={'100%'} p={'26px 30px 0px 30px'}>
                    <Button outlineBlue largeCancelText sx={{ height: '42px !important', width: '183px !important' }} onClick={() => handleButtonClick()}>Add Attachment</Button>
                    <input type="file" ref={fileInputRef} style={{ display: 'none' }} onChange={handleFileChange} />
                    <Stack direction={'row'} gap={'20px'} >
                        <Button blackCancelBtn redHover onClick={() => handleClose()}>Cancel</Button>
                        <Button blueBtnSave onClick={() => ''}>Send</Button>
                    </Stack>
                </Stack>
            </Box>
    ...
    const PaymentRecordView = () => {
        return (
            <Box sx={{ width: '828px !important', padding: '25px' }}>
                <Stack direction={'row'} width={'100%'} justifyContent={'space-between'} mb={4} >
                    <Text className={classes.heading1}>{`Payment Record of ${invoiceForm.reference_id}`}</Text>
                    <CloseIcon onClick={() => handleClose()} style={{ cursor: 'pointer' }} />
                </Stack>
                        ...
                        <Box sx={{ height: '80px', }}>
                            <Input
                                ...
                                inputProps={{
                                    ...
                                    value: paymentRecordData.total_excess_amount,
                                    disabled: true
                                }}
                    ...
                    <Grid item lg={6} md={6} sm={12} xs={12}>
                        <Box sx={{ height: '80px', }}>
                            ...
                            <DatePicker
                                ...
                                labelText={<Text largeLabel>Payment Date</Text>}
                            />
        ...
        <Box className={classes.containerMain} >
            <Box width={'100%'} mb={'20px'} sx={{ display: 'flex', justifyContent: 'space-between' }}>
                <Breadcrumbs aria-label="breadcrumb">
                    <Link to='/sales/invoices' className={classes.linkStyle}><Text className={classes.navText1}>Sales</Text></Link>
                    <Link to='/sales/invoices' className={classes.linkStyle}><Text className={classes.navText1}>Invoices</Text></Link>
                    <Link to='' className={classes.navText2}><Text className={classes.navText1}>Invoices History</Text></Link>
                </Breadcrumbs>
            </Box>
            ...
            <Grid container columnSpacing={2}>
                ...
                <Grid item lg={8}>
                        ...
                        <Box className={classes.ViewContainer}>
                            ...
                            <Box className={classes.containerHeader}>
                                ...
                                <Stack direction={'row'} gap={'18px'} alignItems={'center'}>
                                    ...
                                    <Box width={'159px'}>
                                        {['Approved', "Partially Paid"].includes(invoiceForm.status) ?
                                            ...
                                            <>
                                                <Button
                                                    ...
                                                    endIcon={selectOpen ? <KeyboardArrowUpIcon /> : <KeyboardArrowDownIcon />}
                                                >
                                                    {selectValue}
                                                </Button>
                                                ...
                                                >

                                                    {
                                                        paymentOptions.map((option) => (
                                                            <MenuItem key={option.id} onClick={() => handlePaymentMethod(option.value)} className={classes.viewText} >{option.value}</MenuItem>
                                                        ))
                                                    }
                                    ...
                                    {
                                        invoiceForm.edit_access ? <img src={EditIcon} className={classes.iconStyles} onClick={() => navigate('/sales/add-invoices', { state: { id: selected, action: 'edit', pagination: pagination, company_id: location.state.data.company_id } })} alt='icons' /> : null
                                    }
                                    {
                                    ...
                                    >

                                        {
                                            statusOptions.map((option) => {
                                                ...
                                                } else if (option.value === "Sent") {
                                                    return !invoiceForm.mark_as_sent && <MenuItem key={option.id} onClick={() => handleStatus(option.id)} className={classes.menuText} >{option.value}</MenuItem>
                                                } else {
                                                    return <MenuItem key={option.id}
                                                        onClick={
                                                            () => {
                                                                if (option.value == "Clone") {
                                                                    if (invoiceForm.ledger_item_details.some(i => i.timesheets_available == true)) {
                                                                        addWarningMsg("Generating a clone is prohibited since this invoice is created in correlation with the associated timesheets.")
                                                                    } else {
                                                                        if (["Write Off", "Void"].includes(invoiceForm.status)) {
                                                                            // addWarningMsg("This invoice has been marked as 'write off' or 'void' status. Cloning this invoice is prohibited.")
                                                                            addWarningMsg(`Performing invoice Cloning is not permitted when the invoice status is 'write off' or 'void'.`);
                                                                        } else {
                                ...
                                <Box className={classes.template}>
                                    <Stack direction={'row'} justifyContent={'space-between'} width={'100%'} alignItems={'center'} mb={'30px'}>
                                        ...
                                        <Stack width={'200px'} alignItems={'end'}>
                                            <Text className={classes.heading2}>Invoice No</Text>
                                        ...
                                        <Stack width={'200px'} gap={1}>
                                            <Text className={classes.headingFrom}>{`Spirit (From)`}</Text>
                                            {
                                                ...
                                                invoiceForm ? invoiceForm.billing_address ? invoiceForm.billing_address.length > 0 ?
                                                    ...
                                                    ${invoiceForm.organization_address[0].address_line_one ? invoiceForm.organization_address[0].address_line_one : ''}
                                                            ${invoiceForm.organization_address[0].address_line_two ? invoiceForm.organization_address[0].address_line_two : ''}
                                                            ${invoiceForm.organization_address[0].city ? invoiceForm.organization_address[0].city : ''}
                                                            ${invoiceForm.organization_address[0].state_name ? invoiceForm.organization_address[0].state_name : ''} ,
                                                            ${invoiceForm.organization_address[0].country_name ? invoiceForm.organization_address[0].country_name : ''} 
                                                            ${invoiceForm.organization_address[0].zip_code ? invoiceForm.organization_address[0].zip_code : ''}.
                                        ...
                                        <Stack width={'200px'} gap={1} alignItems={'end'}>
                                            ...
                                            <Text className={classes.headingTo}>{`${invoiceForm.name ? invoiceForm.name : '--'} (To)`}</Text>
                                            {
                                                ...
                                                invoiceForm ? invoiceForm.shipping_address ? invoiceForm.shipping_address.length > 0 ?
                                                    ...
                                                    ${invoiceForm.billing_address[0].address_line_one ? invoiceForm.billing_address[0].address_line_one : ''}
                                                    ${invoiceForm.billing_address[0].address_line_two ? invoiceForm.billing_address[0].address_line_two : ''}
                                    ...
                                    <Stack className={classes.dateData} direction={'row'}>
                                        ...
                                        <Text className={classes.heading4}>{`Invoice Date : ${invoiceForm.date ? invoiceForm.date : '--'}`}</Text>
                                    ...
                                    <Stack gap={1} mt={'30px'} >
                                        ...
                                        <Text className={classes.heading2}>Documents: </Text>
                                        {
                                            ...
                                            invoiceForm.documents.map((doc, i) => (
                                                <span>
                                                    {/* <Link to={doc.document_url} target="_blank" className={`${doc.document_url ? classes.doclink : classes.doclinkEmpty}`}>
                    ...
                    {loading ?
                        null :
                        (invoiceForm.status !== "Rejected" && invoiceForm.approval_access) ?
                            <>
                                <Stack direction={'row'} justifyContent={'end'} my={'20px'} gap={'20px'}>
                                    {
                                        (LocalStorage.getUserData().super_admin || (rolePermission.some(item => item.slug == "invoice_payment_edit" && item.is_allowed == true))) ?
                                            <Button cancelOutline onClick={() => { setPopup('reject'); setOpen(true) }}>
                                                Reject
                                            </Button> :
                                            <Button rejectDisable>
                                                Reject
                                            </Button>
                                    }
                                    {
                                ...
                                <Stack direction={'row'} justifyContent={'end'} my={'20px'} gap={'20px'}>
                                    {["Approval In Progress", "Submitted"].includes(invoiceForm.status) ?
                                        (LocalStorage.getUserData().super_admin || (rolePermission.some(item => item.slug == "invoice_payment_edit" && item.is_allowed == true))) ?
                                            ...
                                            <Button addButtonDisable endIcon={<BellIcon />}>
                                                Send Remainder
                                            </Button>
                                        : null}
                        ...
                        <Stack direction={'row'} justifyContent={'space-between'} px={'15px'}>
                            <Box sx={{ width: '231px' }}>
                                <Tabs
                                    ...
                                    indicatorColor={''}
                                >
                                    <Tab label={'Client History'}  {...a11yProps(0)} className={`${classes.tabTitle1}  ${cardTab === 0 ? classes.activeTabTitle1 : null}`} />
            ...
            >
                <DialogContent >
                    ...
                    {
                        popup === 'invoice_mail' ? InvoiceMailView() : null
                    }

                    {
                        popup === 'write_off' ? WriteOffView() : null
                    }
