...
const dataArr = ["Overview", "Transactions", "Company Details", "Contact Details", "TimeSheet Configuration", "Invoice Configuration"];
...
export default function UserProfile() {
    ...
    const { name, reference_id, document_url, profile_perecentage, status, id } = location.state.data;
    ...
    React.useEffect(() => {
        ...
        setStatuss(status);
        if ((!LocalStorage.getUserData().super_admin) && (rolePermission.some(item => item.slug == "client_overview" && item.is_allowed !== true))) {
        ...
        if ((!LocalStorage.getUserData().super_admin) && (rolePermission.some(item => item.slug == "client_transactions" && item.is_allowed !== true))) {
            for (let i = 0; i < dataArr.length; i++) {
                if (dataArr[i] == 'Transactions') {
                    dataArr.splice('Transactions', 1)
                }
            }
        }
    ...
    const updateClientStatus = () => {
        ...
        ClientsAPI.updateStatus("client", location.state.data.id, data).then((response) => {
            setTimeout(() => {
                setStatusLoading(false)
                if (response.data.statusCode == 1003) {
                    addSuccessMsg(`User Status has been ${statuss == 'Active' ? 'In Activated' : 'Activated'} Successfully`);
                    setStatuss(statuss == 'Active' ? 'In Active' : 'Active');
                    let sideData = location.state.data
                    sideData.status = statuss == 'Active' ? 'In Active' : 'Active'
                    console.log(sideData, 'sideData')
                    navigate('/clients/clients-user-profile', { state: { data: sideData, viewState: 'view' } });
    ...
    const uploadLogo = (id) => {
        ...
        ClientsAPI.uploadClientLogo("client", location.state.data.id, data).then((response) => {
            setLoading(false)
            if (response.data.statusCode == 1003) {
        ...
        <Box className={classes.mainContainer} px={5} py={1}>
            <Box mx={2}>
                <Breadcrumbs aria-label="breadcrumb">
                    <Typography component={Link} to={'/clients'} className={classes.breadcrumbsLink}>Clients</Typography>
                    <Typography className={classes.breadcrumbsName}>Clients User Profile</Typography>
                </Breadcrumbs>
            </Box>
            <Box my={2} mx={2}>
                <Grid container spacing={0}>
                    <Grid item container lg={3} md={3} sm={4} xs={12} pr={2}>
                        <Box className={classes.cardBg}>
                            <Box width={"100%"} display={'flex'} justifyContent={'end'} px={2} pt={2}>
                                ...
                                >
                                    <MenuItem onClick={() => openDeletPop(!statuss)} className={classes.inactive}>
                                        <span>{statuss == 'In Active' ? 'In Active' : 'Active'}</span>
                            ...
                            <Grid item lg={12} px={3} textAlign='center' mt={1}>
                                <Text largeBlack noWrap>{name}</Text>
                            </Grid>
                            <Grid item lg={12} px={3} textAlign='center' py={2}>
                                <Text largeBlack noWrap>{reference_id}</Text>
                            </Grid>
                            <Box px={3}>
                                <Divider />
                            </Box>
                    ...
                    <Grid item lg={9} md={9} sm={8} xs={12} pl={2}>
                        <Box className={classes.cardBg}>
                            {
                                current === "Overview" ? <Overview id={id} /> : null
                            }
                            {
                                current === "Transactions" ? <Transactions /> : null
                            }
                            {
                                current === "Company Details" ? <CompanyDetails /> : null
                            }
                            {
                                current === "Contact Details" ? <ContactDetails /> : null
                            }
                            {
                                current === "TimeSheet Configuration" ? <TimeSheetConfiguration /> : null
                            }
                            {
                                current === "Invoice Configuration" ? <InvoiceConfiguration /> : null
                            }
                        </Box>
                    </Grid>
                        ...
                        <Box sx={{ margin: "20px", }}>
                            ...
                            <Box my={3}>
                                ...
                                <Typography my={1} sx={{ color: "#54595E99", font: '14px  Nunito , Nunito Sans, sans-serif', fontWeight: 400, textAlign: 'center' }}>
                                    Do You Really Wish To {status == 'Active' ? 'In Activate' : 'Activate'} the Client.
                                </Typography>
                            </Box>
                            ...
                            <Box my={2} sx={{ width: '100%', display: 'flex', justifyContent: 'center', gap: '20px' }} >
                                ...
                                <CustomButton popupDelete onClick={updateClientStatus}>
                                    Yes, {statuss == 'Active' ? 'In Activate' : 'Activate'}
                                </CustomButton>
                            </Box>
                        </Box>
                    </ReusablePopup>
