...
export default function ContactDetails() {
    ...
    const getContactsDetails = () => {
        setLoading(true);
        ClientsAPI.getContactsDetails('end-client', location.state.data.id).then((response) => {
            setTimeout(() => {
                setLoading(false)
    ...
    const storeContactsDetails = () => {
        ...
        setLoading(true)
        ClientsAPI.storeContactsDetails("end-client", data,).then((response) => {
            setTimeout(() => {
                setLoading(false)
                if (response.data.statusCode == 1003) {
    ...
    const updateContactsDetails = () => {
        ...
        setLoading(true)
        ClientsAPI.updateContactsDetails("end-client", location.state.data.id, data,).then((response) => {
            setTimeout(() => {
                setLoading(false)
                if (response.data.statusCode == 1003) {
                    getContactsDetails();
    ...
    const deleteContactsDetails = (id) => {
        ...
        setLoading(true)
        ClientsAPI.deleteContactsDetails("end-client", id, data,).then((response) => {
            // setTimeout(() => {
            setLoading(false)
            if (response.data.statusCode == 1003) {
                getContactsDetails();
