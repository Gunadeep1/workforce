...
export default function FilterCard(props) {
    ...
    const { item, filter, getFilterTimesheetData } = props;
    ...
    const filterTimesheet = (e, slug, filterType) => {
        ...
        // }
        // setAnchorEl(null);
        getFilterTimesheetData(e,slug, filterType)
    ...
    return (
        <Box>
            <Box
                className={`${classes.card} ${classes[item.slug]} ${filter.slug === item.slug ? classes[item.slug + "_active"] : ""}`}
                onClick={(e) => item.slug !== "total_invoiced_timesheets" && filterTimesheet(e, item.slug, "all")}
