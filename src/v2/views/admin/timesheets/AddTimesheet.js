                ...
                <Box m={3}>
                            ...
                            <Grid item lg={4} md={4} sm={12} xs={12}>
                                <Box p={1} mt={1} mx={1}>
                                    <FileInput
                                        ...
                                        isDisabled={location.state == null ? false : !formData.is_editable || (formData.approved_status !== 3 && location.state.name == 'Invoice Ready Timesheet' && formData.is_editable && edit)} />
                                ...
                                <Box p={1} mx={1} my={1}>
                                    <Input
                                        ...
                                        inputProps={{
                                            ...
                                            type: 'text',
                                            disabled: location.state == null ? false : !formData.is_editable || (formData.approved_status !== 3 && location.state.name == 'Invoice Ready Timesheet' && formData.is_editable && edit)
                                        }}
                                        handleChange={handleChange}
                                        clientInput
                    ...
                    </Box>
                    {
                        tableHours.map((table, key) => (
                            <Box my={3} key={key}>
                                <TimesheetTable hoursArr={table} handleChangeHours={handleChangeHours} tableIndex={key} editscreen={location.state !== null} location={location} formData={formData} edit={edit} />
                            </Box>
                        ))
                    }
                    <Box>
                        <Box mt={2} px={1} display={"flex"} justifyContent={"end"} gap={3}>
                            ...
                            {location.state !== null ?
                                formData.status !== "Approved" && formData.is_approver ?
                            ...
                            {location.state !== null ?
                                formData.approved_status !== 3 && formData.is_editable && location && location.state && location.state.name != 'Invoice Ready Timesheet' ?
                                    LocalStorage.getUserData().super_admin || (rolePermission.some(item => item.slug == "timesheet_edit" && item.is_allowed == true)) ?
                                        <LoadingButton saveLoader loading={loading} onClick={() => handleSubmit()}>
                                            Submit
                                        </LoadingButton> :
