...
function OnboardEmployee() {
  ...
  const [subStepper, setSubstepper] = useState(0);
    ...
    EmployeesApi.getEmployee(location.state.id).then((response) => {
      setTimeout(() => {
        if (response.data.statusCode == 1003) {
          ...
          statesList(response.data.data.current_address.country_id) // for getting states in current address
          statesList(response.data.data.emergency_contacts[0].country_id,0) // for emergency contacts - 1
          statesList(response.data.data.emergency_contacts[1].country_id,1) // for emergency contacts - 2
