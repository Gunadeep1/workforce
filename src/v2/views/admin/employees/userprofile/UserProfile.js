    ...
    return (
        <Box className={classes.mainContainer} px={5} py={1}>
                                ...
                                <Stack my={1} direction="row" justifyContent={"center"} spacing={2}>
                                    ...
                                    }
                                    {
                                        loading ? <Skeleton width='45px' height='60px' animation='wave' /> :
                                            <Button variant="outlined" className={open ? classes.blueMenu : classes.moreBtn} >
                                                <MoreHorizIcon sx={{ fontSize: "18px", color: open ? `${blue}` : "#9D9E9F" }} onClick={handleClick}/>
                                                <Menu
                                                    id="basic-menu"
                                                    anchorEl={anchorEl}
                                                    open={open}
                                                        ...
                                                        <>
                                                            {
                                                                (LocalStorage.getUserData().super_admin || (rolePermission.some(item => item.slug == "employee_active_inactive" && item.is_allowed == true))) ?
                                                                    <MenuItem className={classes.inactive} onClick={() => handleStatus(0)}>Mark as Active</MenuItem> :
                                                        ...
                                                        <>
                                                            {
                                                                (LocalStorage.getUserData().super_admin || (rolePermission.some(item => item.slug == "employee_disable_enable" && item.is_allowed == true))) ?
                                                                    <MenuItem onClick={() => handleStatus(1)} className={classes.viewText}>{employee.employment_details.enable_login == '1' ? 'Disable' : 'Enable'} User Access</MenuItem> :
                                                                    <MenuItem className={classes.inactiveDisable}>{employee.employment_details.enable_login == '1' ? 'Disable' : 'Enable'} User Access</MenuItem>
                                                            }
                                                            {
                                                                (LocalStorage.getUserData().super_admin || (rolePermission.some(item => item.slug == "employee_active_inactive" && item.is_allowed == true))) ?
                        ...
                        <>
                            ...
                            <Box my={2} sx={{ width: '100%', display: 'flex', justifyContent: 'center', gap: '20px' }} >
                                ...
                                <CustomButton popupSaveBlue component={Link}
                                    ...
                                    state={{
                                        full_name: employee.basic_details.full_name, reference_id: employee.employment_details.reference_id, enable_login: employee.enable_login, avatar_url: employee.profile_picture_url, id: id, acc: 1, visa: employee.employment_details.visa_type,
                                        // getData1:getData 
                                    }}
                                >
                                    Yes
            ...
            <BootstrapDialog
                ...
                sx={{
                    "& .MuiDialog-paper ": {
                        borderRadius: "0px", width: "650px !important", height: "630px !important", overflowY: "auto",
                        '&::-webkit-scrollbar': {
                            display: 'none'
                        },
                    },
