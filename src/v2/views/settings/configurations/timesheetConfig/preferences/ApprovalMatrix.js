...
import { Box, Grid, Chip, Autocomplete, TextField, Skeleton } from '@mui/material'
...
import ApprovalMatrixApi from '../../../../../apis/configurations/timesheet/ApprovalMatrixApi';
import { addErrorMsg, addWarningMsg, addSuccessMsg } from '../../../../../utils/utils';
...
export default function ApprovalMatrix({ current }) {
  ...
  const getIndex = () => {
    setLoading(true)
    ApprovalMatrixApi.indexApi().then((response) => {
      setTimeout(() => {
        setLoading(false)
        if (response.data.statusCode == 1003) {
  ...
  const updateApi = (dataId) => {
    ...
    data['id'] = state.id
    ApprovalMatrixApi.updateApi(data, dataId).then((response) => {
      if (response.data.statusCode == 1003) {
        getIndex()
        getActivity(activityFilter)
  ...
  const storeApi = (data) => {
    ...
    // setLoading(true)
    ApprovalMatrixApi.storeApi(data).then((response) => {
      if (response.data.statusCode == 1003) {
        getIndex()
        getActivity(activityFilter)
        addSuccessMsg(response.data.message);
  ...
  return (
    <Box sx={{
      ...
      <Box className={classes.activeItemBox2}>
                ...
                <>
                  <Grid item lg={12} md={10} sm={9} xs={11}>
                    <Box mt={1}>
                      ...
                      {/* <Text errorText> {approvalsError && approvalsError.length > 0 && approvalsError[key] && approvalsError[key].approvals ? approvalsError[key].approvals : ""}</Text> */}
                    </Box>
                  </Grid>
        ...
        {isEditable ?
          (LocalStorage.getUserData().super_admin || (rolePermission.some(item => item.slug == "configurations_create" && item.is_allowed == true) && rolePermission.some(item => item.slug == "configuration_timesheet" && item.is_allowed == true))) &&
          <Button disable={isEditable} sx={{ width: '100%' }}
          >Add New Level</Button>
          :
          <Button disable={isEditable} addTitleBtn sx={{ width: '100%' }} onClick={() => handleAddLevel()}
          >Add New Level</Button>
        }
