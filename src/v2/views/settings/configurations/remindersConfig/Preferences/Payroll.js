...
function Payroll({ current, activityData, getActivity, activityTotal }) {
    ...
    const [accordionState, setAccordionState] = useState('generatePayroll');

    const handleChangeAccordion = (event, expanded, id) => {
        setAccordionState(expanded ? 'generatePayroll' : '');

        setFormData(prevFormData => ({
            ...prevFormData,
            reminder_name_id: expanded ? id : '11'
        }));
    };
    ...
    const getTemplateSlugs = () => {
        TemplateApi.getTemplateSlug(LocalStorage.uid(), "generate-payroll-reminder", LocalStorage.getAccessToken()).then(
            (res) => {
            ...
            (res) => {
                if (res.data.statusCode == 1003) {
                    if (res.data.data.length > 0) {
                        let ApprovalObj = res.data.data.filter(i => i.reminder_name_id == 11)[0];
        ...
        <Box sx={{
            ...
            <Box className={classes.activeItemBox}>
                ...
                <div style={{ marginTop: '16px' }}>
                    <Accordion onChange={() => handleChangeAccordion(formData.id)} sx={{ border: 'none', boxShadow: 'none' }}>
                        <AccordionSummary
                        ...
                        >
                            <Box display={"flex"} gap={1.5}>
                                ...
                                <Text sx={{ fontFamily: "Nunito , Nunito Sans, sans-serif !important", fontSize: '16px', fontWeight: '600', color: '#092333' }}>Generate Payroll</Text>
                            </Box>
                        </AccordionSummary>
                        <AccordionDetails>
                                        ...
                                        {index === formData.reminders.length - 1 ?
                                            <Box width={'100%'}>
                                                <CustomSelect
                                                    ...
                                                    onChange={(e) => handleChange(e, index)}
                                                />
                                            </Box> : <Box width={'100%'} paddingTop={'10px'}><img onClick={() => handleRemoveReminder(index)} style={{ cursor: 'pointer' }} src={Minus} alt="Minus"></img></Box>
                                        }
                                    </Box>
                                ))}
