...
function Employee({ current, activityData, getActivity, activityTotal }) {
    var rolePermission = LocalStorage.getRolesData() ? LocalStorage.getRolesData().role_permissions.permissions : '';
    const classes = MainStyles()
    console.log(activityData, "activ")
    ...
    const [accordionState, setAccordionState] = useState('profileApproval');// eslint-disable-next-line
    const [invoiceDraftedAccordionState, setInvoiceDraftedAccordionState] = useState('eVerify');// eslint-disable-next-line
    const [invoiceDueAccordionState, setInvoiceDueAccordionState] = useState('personalDocs');

    const handleChangeAccordion = (event, expanded) => {
        setAccordionState(expanded ? 'profileApproval' : '');
    ...
    const handleChangeInvoiceDraftedAccordion = (event, expanded) => {
        ...
        setInvoiceDraftedAccordionState(expanded ? 'eVerify' : '');

        setFormDataSubmission(prevFormDataSubmission => ({
            ...prevFormDataSubmission,
            reminder_name_id: expanded ? '9' : ''
        }));
    };

    const handleChangeInvoiceDueAccordion = (event, expanded) => {

        setInvoiceDueAccordionState(expanded ? 'personalDocs' : '');

        setFormDataDue(prevFormDataDue => ({
            ...prevFormDataDue,
            reminder_name_id: expanded ? '10' : ''
        }));
    };
    ...
    const getTemplateSlugs = () => {
        TemplateApi.getTemplateSlug(LocalStorage.uid(), "employee-profile-approval", LocalStorage.getAccessToken()).then(
            (res) => {
            ...
            (res) => {
                if (res.data.statusCode == 1003) {
                    if (res.data.data.length > 0) {
                        ...
                        let SubmissionObj = res.data.data.filter(i => i.reminder_name_id == 9)[0];
                        let DueObj = res.data.data.filter(i => i.reminder_name_id == 10)[0];
                ...
                <div style={{ marginTop: '16px' }}>
                    <Accordion onChange={handleChangeAccordion} sx={{ border: 'none', boxShadow: 'none' }}>
                        ...
                        >
                            <Box display={"flex"} gap={1.5}>
                                ...
                                <Text sx={{ fontFamily: "Nunito , Nunito Sans, sans-serif !important", fontSize: '16px', fontWeight: '600', color: '#092333' }}>Employee Profile Approval</Text>
                            </Box>
                        </AccordionSummary>
                        <AccordionDetails>
                                        ...
                                        {index === formData.reminders.length - 1 ?
                                            <Box width={'100%'}>
                                                <CustomSelect
                                                    ...
                                                    onChange={(e) => handleChange(e, index, formData.id)}
                                                />
                                            </Box> : <Box width={'100%'} paddingTop={'10px'}><img onClick={() => handleRemoveReminder(index, formData.id)} style={{ cursor: 'pointer' }} src={Minus} alt="Minus"></img></Box>
                                        }
                                    </Box>
                                ))}
                    ...
                    <Accordion onChange={handleChangeInvoiceDraftedAccordion} sx={{ border: 'none', boxShadow: 'none' }}>
                        ...
                        >
                            <Box display={"flex"} gap={1.5}>
                                <img src={DateIcon} alt="DateIcon"></img>
                                <Text sx={{ fontFamily: "Nunito , Nunito Sans, sans-serif !important", fontSize: '16px', fontWeight: '600', color: '#092333' }}>E-Verify</Text>
                            </Box>
                        </AccordionSummary>
                        <AccordionDetails>
                            <Box sx={{ width: '100%', height: '643px', overflowY: 'auto', backgroundColor: '#FBFBFB', borderRadius: '12px', padding: '30px' }}>
                                ...
                                {formDataSubmission.reminders.map((step, index) => (
                                    <Box mt={2} gap={1} sx={{ display: 'flex' }} width={'100%'} >
                                        ...
                                        {index === formDataSubmission.reminders.length - 1 ?
                                            <Box width={'100%'}>
                                                <CustomSelect
                                                    commonSelect
                                                    label={<Text sx={{ fontFamily: "Nunito , Nunito Sans, sans-serif !important", fontSize: '12px', fontWeight: '400', color: '#737373' }}>Remind Every<br />(Optional)</Text>}
                                                    ...
                                                    onChange={(e) => handleChange(e, index, formDataSubmission.id)}
                                                />
                                            </Box> : <Box width={'100%'} paddingTop={'10px'}><img onClick={() => handleRemoveReminder(index, formDataSubmission.id)} style={{ cursor: 'pointer' }} src={Minus} alt="Minus"></img></Box>
                                        }
                                    </Box>
                                ))}
                    ...
                    <Accordion onChange={handleChangeInvoiceDueAccordion} sx={{ border: 'none', boxShadow: 'none' }}>
                        ...
                        >
                            <Box display={"flex"} gap={1.5}>
                                <img src={DateIcon} alt="DateIcon"></img>
                                <Text sx={{ fontFamily: "Nunito , Nunito Sans, sans-serif !important", fontSize: '16px', fontWeight: '600', color: '#092333' }}>Personal Docs</Text>
                            </Box>
                        </AccordionSummary>
                        <AccordionDetails>
                            <Box sx={{ width: '100%', height: '643px', overflowY: 'auto', backgroundColor: '#FBFBFB', borderRadius: '12px', padding: '30px' }}>
