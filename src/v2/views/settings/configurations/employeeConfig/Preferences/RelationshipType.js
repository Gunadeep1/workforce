...
import RelationshipTypeApi from '../../../../../apis/configurations/employee/RelationshipTypeApi';
...
export default function RelationshipType({ current }) {
  ...
  useEffect(() => {
    setTimeout(() => {
      RelationshipTypeListing(filter)
    ...
    if (current) {
      ...
      if (scrollTop + clientHeight >= scrollHeight) {
        ...
        if (total >= filter.limit) {
          ...
          RelationshipTypeListing({ ...filter, limit: filter.limit + 10, page: 1, });
        }
  ...
  const searchAPICall = (text) => {
    RelationshipTypeApi.getRelationashipTypeListing({ ...filter, search: text }).then((res) => {
  ...
  const closeBtn = () => {
    ...
    setTimeout(() => {
      RelationshipTypeApi.getRelationashipTypeListing({ ...filter, search: "" }).then((res) => {
        setLoading(false);
        setGetData(res.data.data);
        // setPagination(res.data.pagination);
      })
  ...
  const RelationshipTypeListing = (args) => {
    if (filter.limit <= 10) {
      setLoading(true)
    } else {
      setLoading(false)
    }
    RelationshipTypeApi.getRelationashipTypeListing(args).then((response) => {
      setTimeout(() => {
        setLoading(false)
        if (response.data.statusCode == 1003) {
  ...
  const updateStatus = (args) => {
    ...
    RelationshipTypeApi.updateStatus(data, args.id).then((response) => {
      if (response.data.statusCode == 1003) {
  ...
  const storeRelationshipType = (data) => {
    data.request_id = LocalStorage.uid()
    setLoading(true)
    RelationshipTypeApi.storeRelationshipType(data).then((response) => {
      setTimeout(() => {
        setLoading(false)
        if (response.data.statusCode == 1003) {
          RelationshipTypeListing(filter)
  ...
  const updateApi = (dataId) => {
    ...
    setLoading(true)
    RelationshipTypeApi.update(data, dataId).then((response) => {
      setTimeout(() => {
        setLoading(false)
        if (response.data.statusCode == 1003) {
          RelationshipTypeListing(filter)
  ...
  const deleteTeam = (dataId) => {
    ...
    RelationshipTypeApi.deleteApi(data, dataId).then((response) => {
      if (response.data.statusCode == 1003) {
        RelationshipTypeListing(filter)
        getActivity(activityFilter)
        addSuccessMsg(response.data.message);
            ...
            loading ? <>
              ...
              ))}
              {
                (LocalStorage.getUserData().super_admin || (rolePermission.some(item => item.slug == "configurations_create" && item.is_allowed == true) && rolePermission.some(item => item.slug == "configuration_employee" && item.is_allowed == true))) ?
                  <Button addTitleBtn onClick={() => handleClickOpen()}>Add Relationship Type</Button> :
                  <Button addTitleBtnDisable>Add Relationship Type</Button>
              }
            </>
              : <>
                ...
                <Button addTitleBtn sx={{ width: '100%' }} onClick={() => handleClickOpen()}>Add Relationship Type</Button>
        ...
        >
          <DialogContent >
            ...
            <Box padding={'38px 30px 35px 30px '}>
              <Box mb={4}>
                <Text mediumViewAmt>Relationship</Text>
              </Box>
                  ...
                  <Grid container spacing={6}>
                    ...
                    <Grid item lg={8} md={8} sm={6} xs={12} display={"flex"} justifyContent={'center'} flexDirection={"column"}>
                      {value.action_type_name === "update" ?
                        <Text BrowmnMnStepperText>{value.field_changes}</Text> : value.action_type_name === "store" ? <Text BrowmnMnStepperText>{value.created_by} stored the new Relationship Type {value.referrable_name}</Text> : value.action_type_name === "delete" ? <Text BrowmnMnStepperText>{value.created_by} has deleted Relationship Type {value.referrable_name}</Text> : null
                      }
