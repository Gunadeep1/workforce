...
import DepartmentApi from '../../../../../apis/configurations/employee/DepartmentApi';
...
function Department({ current }) {
  ...
  const handleSaveAndEdit = () => {
    if (isEditable) {
      ...
      if (state.id != "") {
        updateDepartment(state.id)
      } else {
        storeDepartment(state)
        handleSubmit()
      }
  ...
  // const getDepartmentIndex = (id) => {
  //   DepartmentApi.getDepartmentIndex(
  //     id
  //   ).then((response) => {
  //     setTimeout(() => {
  //       if (response.data.statusCode == 1003) {
  ...
  //   });
  // }
  const deleteDepartment = (dataId) => {
    let data = { request_id: LocalStorage.uid() };
    DepartmentApi.deleteDeparment(data, dataId).then((response) => {
      if (response.data.statusCode == 1003) {
        getDepartmentListing(filter)
  ...
  const storeDepartment = (data) => {
    data.request_id = LocalStorage.uid()
    setLoading(true)
    DepartmentApi.postDepartment(data).then((response) => {
      setTimeout(() => {
        setLoading(false)
        if (response.data.statusCode == 1003) {
          getDepartmentListing(filter)
  ...
  const updateDepartment = (dataId) => {
    let data = { ...state, request_id: LocalStorage.uid() };

    setLoading(true)
    DepartmentApi.updateDepartment(data, dataId).then((response) => {
      setTimeout(() => {
        setLoading(false)
        if (response.data.statusCode == 1003) {
          getDepartmentListing(filter)
  ...
  const updateStatus = (args) => {
    ...
    DepartmentApi.updateDepartmentStatus(data, args.id).then((response) => {
      if (response.data.statusCode == 1003) {
        addSuccessMsg(response.data.message);
        getDepartmentListing(filter)
        getActivity(activityFilter)
  ...
  const getDepartmentListing = (args) => {
    setLoading(true)
    DepartmentApi.getDepartmentListing(args).then((response) => {
      setTimeout(() => {
        if (response.data.statusCode == 1003) {
          setLoading(false)
  ...
  const closeBtn = () => {
    ...
    setTimeout(() => {
      DepartmentApi.getDepartmentListing({ ...filter, search: "" }).then((res) => {
        setLoading(false);
    ...
    if (current) {
      ...
      if (scrollTop + clientHeight >= scrollHeight) {
        ...
        if (total >= filter.limit) {
          ...
          getDepartmentListing({ ...filter, limit: filter.limit + 10, page: 1, });
        }
  ...
  const searchAPICall = (text) => {
    DepartmentApi.getDepartmentListing({ ...filter, search: text }).then((res) => {
            ...
            <>
              ...
              ))}
              {
                (LocalStorage.getUserData().super_admin || (rolePermission.some(item => item.slug == "configurations_create" && item.is_allowed == true) && rolePermission.some(item => item.slug == "configuration_employee" && item.is_allowed == true))) ?
                  <Button addTitleBtn onClick={() => handleClickOpen()}>Add Department</Button> :
                  <Button addTitleBtnDisable>Add Department</Button>
              }
            </>
          }
        </Box>
          ...
          >
            <DialogContent >
              ...
              <Box padding={'38px 30px 35px 30px '}>
                <Box mb={4}>
                  <Text mediumViewAmt>Department</Text>
                </Box>
                    ...
                    <Grid item lg={8} md={8} sm={6} xs={12} display={"flex"} justifyContent={'center'} flexDirection={"column"}>
                      {value.action_type_name === "store" ?
                        <Text BrowmnMnStepperText>
                          {value.referrable_name === null ? 'Department' : value.referrable_name} has been created </Text> : <Text BrowmnMnStepperText>{value.field_changes === '[]' ? '' : value.field_changes}</Text>}
